...
export default function VendorPaymentDetails({ navigateTo, vendorId }) {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch all bills to filter purchases for this vendor
  ...
  const allocatedBills = useMemo(() => {
    ...
    let remainingPayment = payments.reduce((sum, p) => sum + (p.amount || 0), 0);
  ...
  const totalPurchase = useMemo(() => filteredBills.reduce((sum, b) => sum + (b.final_bill_amount || 0), 0), [filteredBills]);
  ...
  const globalTotalPending = globalTotalPurchase - globalTotalPaid;
    ...
    <div className="w-full px-6 space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Vendor Payment Details</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 shadow-xl">
          <p className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-2">Overall Pending Bill</p>
          <div className="text-3xl font-bold text-rose-400">₹{globalTotalPending.toLocaleString('en-IN')}</div>
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 shadow-xl">
          <p className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-2">
            {!isFiltered ? 'Total Purchase' : 'Period Purchase'}
          </p>
          <div className="text-3xl font-bold text-emerald-400">₹{totalPurchase.toLocaleString('en-IN')}</div>
          <p className="text-xs text-slate-500 mt-2">Sum of {filteredBills.length} purchase bills</p>
        </div>

        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 shadow-xl">
          <p className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-2">
            {!isFiltered ? 'Total Paid' : 'Period Paid'}
          </p>
          <div className="text-3xl font-bold text-indigo-400">₹{totalPaid.toLocaleString('en-IN')}</div>
          <p className="text-xs text-slate-500 mt-2">Sum of {filteredPayments.length} recorded payments</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-700/50 shadow-xl flex flex-col max-h-[500px]">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-lg font-bold text-slate-50">Payment History</h2>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {filteredPayments.map((pay) => (
                  <div key={pay.id} className="bg-slate-900/80 border border-slate-700 p-4 rounded-xl flex items-center justify-between group transition hover:border-slate-500/50">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-slate-200">₹{pay.amount.toLocaleString('en-IN')}</span>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-700/50 shadow-xl flex flex-col max-h-[600px]">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-lg font-bold text-slate-50">Purchase Bills</h2>
          </div>

          <div className="flex-1 overflow-y-auto pr-2">
            {filteredBills.length === 0 ? (
              <div className="text-center py-10 text-slate-500 text-sm">No purchase bills found in this period.</div>
            ) : (
                ...
                {filteredBills.map((bill) => (
                  <div key={bill.id} className="bg-slate-900/80 border border-slate-700 p-4 rounded-xl flex items-center justify-between gap-6">
                    ...
                    <div className="w-28 shrink-0">
                      <div className="font-bold text-emerald-400 text-lg">₹{(bill.final_bill_amount || 0).toLocaleString('en-IN')}</div>
                    ...
                    <div className="w-32 shrink-0 text-xs border-r border-slate-700/50 pr-4">
                      <div className="flex justify-between mb-1.5">
                        ...
                        <span className="text-emerald-400 font-medium">₹{bill.allocatedPaid.toLocaleString('en-IN')}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Pending:</span>
                        <span className="text-rose-400 font-medium">₹{(bill.final_bill_amount - bill.allocatedPaid).toLocaleString('en-IN')}</span>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-slate-50">Record Payment</h2>
