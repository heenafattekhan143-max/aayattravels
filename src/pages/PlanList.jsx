...
export default function PlanList({ navigateTo }) {
  ...
  const startEdit = (plan) => {
    ...
    setEditFormData({
      ...
      company_rate: (plan.company_rate || plan.rate)?.toString() || '',
      vendor_rate: (plan.vendor_rate || plan.rate)?.toString() || '',
  ...
  const validateEditForm = () => {
    ...
    if (!editFormData.company_rate || isNaN(editFormData.company_rate) || parseFloat(editFormData.company_rate) <= 0) {
      tempErrors.company_rate = "Company base rate must be a number greater than 0.";
    }

    if (!editFormData.vendor_rate || isNaN(editFormData.vendor_rate) || parseFloat(editFormData.vendor_rate) <= 0) {
      tempErrors.vendor_rate = "Vendor base rate must be a number greater than 0.";
    }
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-700 bg-table-header text-slate-400 text-xs font-semibold uppercase tracking-wider">
                  ...
                  <th className="p-4">Company Rate</th>
                  <th className="p-4">Vendor Rate</th>
                ...
                {filteredPlans.length === 0 ? (
                  ...
                  filteredPlans.map((plan) => (
                    <tr key={plan.id} className="hover:bg-slate-800/20 transition">
                      ...
                      <td className="p-4 font-bold text-emerald-400">₹{(plan.company_rate || plan.rate || 0).toLocaleString()}</td>
                      <td className="p-4 font-bold text-indigo-300">₹{(plan.vendor_rate || plan.rate || 0).toLocaleString()}</td>
              ...
              {editFormData.plan_type === 'Local' ? (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-slate-300">Company Rate *</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-slate-300">Vendor Rate *</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-slate-300">Company /Day *</label>
                    <input type="text" name="company_rate" value={editFormData.company_rate} onChange={handleEditChange} className={`w-full bg-slate-950 text-xs border ${editErrors.company_rate ? 'border-rose-500' : 'border-slate-700'} rounded-lg px-3 py-2 text-slate-100 outline-none`} />
                    {editErrors.company_rate && <p className="text-[10px] text-rose-400">{editErrors.company_rate}</p>}
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-slate-300">Vendor /Day *</label>
                    <input type="text" name="vendor_rate" value={editFormData.vendor_rate} onChange={handleEditChange} className={`w-full bg-slate-950 text-xs border ${editErrors.vendor_rate ? 'border-rose-500' : 'border-slate-700'} rounded-lg px-3 py-2 text-slate-100 outline-none`} />
                    {editErrors.vendor_rate && <p className="text-[10px] text-rose-400">{editErrors.vendor_rate}</p>}
                  </div>
                </div>
